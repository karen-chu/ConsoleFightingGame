        ...
        public override bool CalculateHealth(int computer_action, int user_action)
        {
            ...
            if (computer_action > 1 && user_action > 1)
            {
                ...
                if (computer_action == 2)
                {
                    ...
                    // Since Computer is a rogue, calculate 20% chance of critical hit
                    int critical_hit = rand.Next(1, 6);
                    if (critical_hit == 1)
                    {
            ...
            else
            {
                ...
                if (Base.user_character == 3)
                {
                    ...
                    {
                        computer_health -= computer_melee_damage + getUniquePower();
                        Console.WriteLine(string.Format("Tank special armor rebounds attack. {0} damage against computer.", computer_melee_damage + getUniquePower()));
                    }
                }
                else
                {
